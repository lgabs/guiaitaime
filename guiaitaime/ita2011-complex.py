...
class WriteStuff(Scene):
    def construct(self):
        example_text = TextMobject(
            ...
            tex_to_color_map={"soma": YELLOW}
        )
        example_tex = TexMobject(
            "z^2+\\abs{z}^2 + iz - 1 = 0",
        )
        example_text2 = TextMobject(
            "em $\mathbb{C}$ é igual a:"
        )
        ...
        group = VGroup(example_text, example_tex, example_text2, options)
        group.arrange(DOWN)
        group.set_width(FRAME_WIDTH - 2 * MED_LARGE_BUFF)
